...
import { createSwarmBeadsProjector } from "./beads";
import { attachSwarmDashboard } from "./dashboard";
import { createHerdrSwarmSession, type HerdrControl } from "./herdr";
import { createSwarmRunManifest } from "./manifest";
import { PipelineController } from "./pipeline";
import { resolveSwarmPlan, type SwarmPlan } from "./plan";
import type { SwarmPluginLogger } from "./plugins";
import { fingerprintSwarmDefinition } from "./schema";
...
function existingRunResult(status: string, swarmName: string): ClaimedSwarmResult {
	if (status === "running")
