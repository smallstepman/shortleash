...
import type { SwarmBeadsProjector, SwarmProjectionEvent } from "./beads";
...
import { resolveSwarmHistoryInheritance, resolveSwarmIsolation, type SwarmAgent, type SwarmDefinition } from "./schema";
		...
		for (let waveIdx = 0; waveIdx < this.#waves.length; waveIdx++) {
			...
			const waveResults = await mapWithConcurrency(
				...
				async ({ agentName, index: currentIndex }) => {
					...
					const signalScope = createAgentSignal(options.signal, this.#def.agentTimeoutMs);
					try {
...
function createAgentSignal(
	parent: AbortSignal | undefined,
	timeoutMs: number | undefined,
): { signal?: AbortSignal; dispose: () => void } {
	if (!parent && timeoutMs === undefined) return { signal: undefined, dispose: () => {} };
...
async function mapWithConcurrency<T, R>(
	...
	const workers = Array.from({ length: Math.min(items.length, Math.max(1, limit)) }, () => worker());
