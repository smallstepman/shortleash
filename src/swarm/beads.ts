...
import { validateSwarmMetadata } from "./metadata";
import { parseSwarm, type SwarmDefinition } from "./schema";
...
export interface SwarmBeadsProjector {
	...
	reconcile(authoritativeStatus: string): Promise<SwarmBeadsReconciliation>;
}
...
export function createSwarmBeadsProjector(
	...
	return {
		...
		async reconcile(authoritativeStatus: string): Promise<SwarmBeadsReconciliation> {
			const bead = await readBead(beadId, cwd, run);
			if (!bead) {
				return {
					beadId,
