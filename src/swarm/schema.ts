...
export interface SwarmAgent {
	...
	checks: SwarmPolicyRef[];
	evals: SwarmPolicyRef[];
}
...
export function parseSwarm(content: string): SwarmDefinition {
	...
	const swarm = isRecord(raw) && isRecord(raw.swarm) ? (raw.swarm as unknown as RawSwarmConfig) : undefined;
	if (!swarm) {
		throw new Error("Swarm definition must have a top-level 'swarm' key");
	}
	rejectRemovedPolicyFields(swarm as unknown as Record<string, unknown>, "swarm");
	...
	if (swarm.task !== undefined && (typeof swarm.task !== "string" || swarm.task.trim().length === 0)) {
		throw new Error("swarm.task must be a non-empty string when provided");
	}
	if (swarm.agents !== undefined && (!isRecord(swarm.agents) || Object.keys(swarm.agents).length === 0)) {
		throw new Error("swarm.agents must contain at least one agent when provided");
	}

	const mode = swarm.mode ?? "sequential";
	if (typeof mode !== "string" || !VALID_MODES.has(mode)) {
		throw new Error(`Invalid mode '${String(mode)}'. Must be one of: ${[...VALID_MODES].join(", ")}`);
	}
	...
	for (const [name, config] of Object.entries(swarm.agents ?? {})) {
		rejectRemovedPolicyFields(config as unknown as Record<string, unknown>, `swarm.agents.${name}`);
		...
		agents.set(name, {
			...
			workspaceIsolation: parseIsolationMode(
				...
				`swarm.agents.${name}.isolation`,
			),
			inheritHistory: parseHistoryInheritance(
				firstDefined(config.inherit_history, config.history, config.parent_history),
				`swarm.agents.${name}.inherit_history`,
			),
			waitsFor: Array.isArray(config.waits_for) ? config.waits_for : [],
			checks: [...checks, ...parsePolicyRefs(config.checks, `swarm.agents.${name}.checks`)],
			evals: [...evals, ...parsePolicyRefs(config.evals, `swarm.agents.${name}.evals`)],
		});
	}
	...
	return {
		...
		task: typeof swarm.task === "string" ? swarm.task.trim() : undefined,
		workspaceIsolation,
		inheritHistory,
